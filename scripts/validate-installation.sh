...
if [ -f ~/.config/sway/config ]; then
    echo -n "Validating Sway config syntax... "
    # Note: sway --validate may show DRM/display errors when run outside a graphical session
    # Filter these out to only show actual configuration syntax errors
    VALIDATION_OUTPUT=$(sway --validate 2>&1)
    SYNTAX_ERRORS=$(echo "$VALIDATION_OUTPUT" | grep -i "error" | grep -v "permission denied\|atomic\|HDMI\|DRM\|connector\|Failed to open\|could not open DRM device")
    
    if [ -z "$SYNTAX_ERRORS" ]; then
        echo "$PASS"
        if echo "$VALIDATION_OUTPUT" | grep -qi "permission denied\|atomic\|DRM"; then
            echo "  ℹ️  Note: DRM/display warnings are normal when validating outside a graphical session"
        fi
    else
        echo "$FAIL"
        echo ""
        echo "Sway config has syntax errors:"
        echo "$SYNTAX_ERRORS" | head -10
        ((ERRORS++))
        FIXES+=("Sway config syntax|Check ~/.config/sway/config for errors")
    fi
else
    echo "$FAIL Sway config missing"
    ((ERRORS++))
    FIXES+=("Sway config missing|cd ~/Projects/bunkeros && ./setup.sh")
fi
